...
const selectEquipePessoaColumns = "equipe_pessoa_id, equipe_id, funcionario_id";
...
export async function listEquipePessoas(equipeId?: string): Promise<EquipePessoaRow[]> {
  ...
  const r = await request.query<EquipePessoaRow>(
    `SELECT ${selectEquipePessoaColumns} FROM dbo.ESCALA_equipe_pessoas${where} ORDER BY equipe_pessoa_id`
  );
  return r.recordset ?? [];
}
